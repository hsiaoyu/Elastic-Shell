...
int main() {
	...
	NTri=F.rows();
	NNode=Vbar.rows();	
	MatrixXd FF(NNode,3),V0(NNode,2), Epnt(3*NTri,3), Epnt2(3*NTri,3), EN(3*NTri,3), FN(NTri,3), ENbar(3*NTri,3), FNbar(NTri,3), Vnew(NNode,3);
	...
	EN=Enormal(FN,EdgeF);		
	FF=Force(constE, nu, t, F, V0, Vbar, V, ENbar, EN, EdgeNV);

/*
//-------------Calculate finite difference E--------------------------------------------------------------------	
	igl::readOBJ("V2.obj",V2,F);
	MatrixXi EdgeNV(3*NTri,4);
	...
	Ei=TotEnergy(constE, nu, t, F, V0, Vbar, V2, ENbar, EN2);
	Ef=TotEnergy(constE, nu, t, F, V0, Vbar, V, ENbar, EN);
	cout << "Theoretical dE" << endl << "dE1       dE2" << endl <<  Ef(0)-Ei(0) << "       "<< Ef(1)-Ei(1) << endl;
	FFtot=Force(constE, nu, t, F, V0, Vbar, V2, ENbar, EN2, EdgeNV);   //Change the output for Force to FF << FF1, FF2;
	dV=V-V2;
	dE1=0;
	dE2=0;
	for (i=0; i<NNode; i++){
...
MatrixXd Force(double constE, double nu, double t, MatrixXi F, MatrixXd V0, MatrixXd Vbar, MatrixXd V, MatrixXd ENbar, MatrixXd EN, MatrixXi EdgeNV){
