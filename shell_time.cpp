...
double area; // area of the canonical triangle
MatrixXd V, Vbar, aV(100,3);
MatrixXi F;
int NNode,cframe=-1;
int main() {
	CanonM3 << 1, 0, 0, 0;
	...
	Vector2d newV;
	Vector3d Min, Max;
	MatrixXd dI(18,2), dV, V2;
	igl::readOBJ("Vbar.obj", Vbar, F);
	igl::readOBJ("V.obj",V,F);
	NTri=F.rows();
	NNode=Vbar.rows();	
	MatrixXd FFtot(NNode,3), Epnt(3*NTri,3), Epnt2(3*NTri,3), EN(3*NTri,3), FN(NTri,3), ENbar(3*NTri,3), FNbar(NTri,3), Vnew(NNode,3);
	VectorXi EdgeF(3*NTri);
	EdgeF=NeighborF(F);

	VectorXd Mass(NNode);
        double  rho = 1;
        Mass = VMass(rho, F, Vbar);
	
//--------------Leap Frog Methods for the change in the vertices--------------------------------------------------
	double delt;
	cout << "Please enter step size:"<< endl;
        cin >> delt;
        int T;
...
//---------------------------------------------------------------------------------------------------------------
/*
//------------------- Finite difference test for the derivative of energy w.r.t the vertices(Force)---------------
        igl::readOBJ("V2.obj",V2,F); //  V is the perturb state and V2 is the initial state
	MatrixXi EdgeNV(3*NTri,4);
	MatrixXd EN2(3*NTri,3), FN2(NTri,3);
	MatrixXd FFtot(2*NNode,3);
	EdgeNV=VofEdgeN(F,EdgeF);
	igl::per_face_normals(Vbar, F, FNbar);
	igl::per_face_normals(V, F, FN);
	igl::per_face_normals(V2, F, FN2);
	ENbar=Enormal(FNbar,EdgeF);		
	EN=Enormal(FN,EdgeF);		
	EN2=Enormal(FN2,EdgeF);		
	Ei=TotEnergy(constE, nu, t, F, Vbar, V2, ENbar, EN2);
	Ef=TotEnergy(constE, nu, t, F, Vbar, V, ENbar, EN);
	cout << "Theoretical dE" << endl << "dE1       dE2" << endl <<  Ef(0)-Ei(0) << "       "<< Ef(1)-Ei(1) << endl;
	FFtot=Force(constE, nu, t, F, Vbar, V2, ENbar, EN2, EdgeNV);   //Change the output for Force to FF << FF1, FF2;
	dV=V-V2;
	dE1=0;
	dE2=0;
	for (i=0; i<NNode; i++){
...
//-----------------Show Force in viewer-----------------------------------------------
	...
	v.setZero();
        aV.block(0,0,NNode,3)=V;
	int frame=1, sampling_rate;
        cout << "Enter Sampling Rate:" << endl;
        cin >> sampling_rate;
	for (i=0; i<T; i++){
		igl::per_face_normals(V, F, FN);
		EN=Enormal(FN,EdgeF);	
		FFtot=Force(constE, nu, t, F, Vbar, V, ENbar, EN, EdgeNV);
		for (j=0; j<NNode; j++){
			v.row(j)+=FFtot.row(j)*delt/Mass(j);
		}
		V+=v*delt;
		if (i%sampling_rate==0){
			cout << frame*NNode << endl;
			aV.block(frame*NNode,0,NNode,3)=V;
			frame+=1;
		}
	}
        cout << aV << endl;
...
bool key_down(igl::viewer::Viewer& viewer, unsigned char key, int modifier){
  ...
  cout << cframe << endl;   
    MatrixXd Vtmp=aV.block(cframe*NNode,0,NNode,3);
    viewer.data.clear();
    viewer.data.set_mesh(Vtmp, F);
    viewer.core.align_camera_center(Vtmp,F);
  //int b = *reinterpret_cast<int*>(&key);
  //b-=49;
  return false;
}
...
VectorXd VMass(double rho, MatrixXi F, MatrixXd V){
	int i;
        double areaT;
        Vector3d e1 , e2;
        VectorXd M(V.rows());
...
Vector3d TotEnergy(double constE, double nu, double t, MatrixXi F, MatrixXd Vbar, MatrixXd V, MatrixXd ENbar, MatrixXd EN){
	int i, j, k, NTri;
	Vector3d E;
	NTri=F.rows();
	E << 0, 0, 0;
...
MatrixXd Force(double constE, double nu, double t, MatrixXi F, MatrixXd Vbar, MatrixXd V, MatrixXd ENbar, MatrixXd EN, MatrixXi EdgeNV){
